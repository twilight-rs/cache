...
#[derive(Clone, Debug)]
pub struct InMemoryGuildRepository(pub(crate) Arc<InMemoryBackendRef>);

impl Repository<GuildEntity, InMemoryBackendError> for InMemoryGuildRepository {
    fn get(&self, guild_id: GuildId) -> GetEntityFuture<'_, GuildEntity, InMemoryBackendError> {
        future::ok(self.0.guilds.get(&guild_id).map(|r| r.value().clone())).boxed()
    }

    fn list(&self) -> ListEntitiesFuture<'_, GuildEntity, InMemoryBackendError> {
        let stream = stream::iter(self.0.guilds.iter().map(|r| Ok(r.value().clone()))).boxed();

        future::ok(stream).boxed()
    }

    fn remove(&self, guild_id: GuildId) -> RemoveEntityFuture<'_, InMemoryBackendError> {
        if !self.0.config.entity_types().contains(EntityType::GUILD) {
    ...
    fn upsert(&self, entity: GuildEntity) -> UpsertEntityFuture<'_, InMemoryBackendError> {
        ...
        self.0.guilds.insert(entity.id(), entity);
...
impl GuildRepository<InMemoryBackendError> for InMemoryGuildRepository {
    fn afk_channel(
        &self,
        guild_id: GuildId,
    ...
    fn channel_ids(
        ...
        let stream = self.0.guild_channels.get(&guild_id).map_or_else(
            || stream::empty().boxed(),
    ...
    fn channels(
        ...
        let channel_ids = match self.0.guild_channels.get(&guild_id) {
        ...
        let iter = channel_ids.into_iter().filter_map(move |id| {
            ...
            if let Some(r) = self.0.channels_voice.get(&id) {
                return Some(Ok(GuildChannelEntity::Voice(r.value().clone())));
            }

            if let Some(r) = self.0.channels_category.get(&id) {
    ...
    fn emoji_ids(
        ...
        let stream = self.0.guild_emojis.get(&guild_id).map_or_else(
            || stream::empty().boxed(),
            |set| stream::iter(set.iter().map(|x| Ok(*x)).collect::<Vec<_>>()).boxed(),
        );
    ...
    fn emojis(
        ...
        let emoji_ids = match self.0.guild_emojis.get(&guild_id) {
        ...
        let iter = emoji_ids
            .into_iter()
            .filter_map(move |id| self.0.emojis.get(&id).map(|r| Ok(r.value().clone())));
        let stream = stream::iter(iter).boxed();
    ...
    fn member_ids(
        ...
        let stream = self.0.guild_members.get(&guild_id).map_or_else(
            || stream::empty().boxed(),
            |set| stream::iter(set.iter().map(|x| Ok(*x)).collect::<Vec<_>>()).boxed(),
        );
    ...
    fn members(
        ...
        let user_ids = match self.0.guild_members.get(&guild_id) {
    ...
    fn presence_ids(
        ...
        let stream = self.0.guild_presences.get(&guild_id).map_or_else(
            || stream::empty().boxed(),
            |set| stream::iter(set.iter().map(|x| Ok(*x)).collect::<Vec<_>>()).boxed(),
        );
    ...
    fn presences(
        ...
        let user_ids = match self.0.guild_presences.get(&guild_id) {
    ...
    fn role_ids(&self, guild_id: GuildId) -> ListEntityIdsFuture<'_, RoleId, InMemoryBackendError> {
        let stream = self.0.guild_roles.get(&guild_id).map_or_else(
            || stream::empty().boxed(),
            |set| stream::iter(set.iter().map(|x| Ok(*x)).collect::<Vec<_>>()).boxed(),
        );
    ...
    fn roles(&self, guild_id: GuildId) -> ListEntitiesFuture<'_, RoleEntity, InMemoryBackendError> {
        let role_ids = match self.0.guild_roles.get(&guild_id) {
    ...
    fn rules_channel(
        ...
        let guild = self
            ...
            .and_then(|id| self.0.channels_text.get(&id))
            .map(|r| r.value().clone());
    ...
    fn voice_state_ids(
        ...
        let stream = self.0.guild_voice_states.get(&guild_id).map_or_else(
            || stream::empty().boxed(),
            |set| stream::iter(set.iter().map(|x| Ok(*x)).collect::<Vec<_>>()).boxed(),
        );
    ...
    fn voice_states(
        ...
        let user_ids = match self.0.guild_voice_states.get(&guild_id) {
    ...
    fn widget_channel(
        ...
        let id = match self.0.guilds.get(&guild_id) {
